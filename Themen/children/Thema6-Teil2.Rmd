...
[Datenquelle](https://raw.githubusercontent.com/avehtari/ROS-Examples/master/KidIQ/data/child_iq.csv)
...
```{r m10-4, echo = TRUE}
m10.4 <- stan_glm(kid_score ~ mom_hs + mom_iq + mom_hs:mom_iq, data = kidiq, refresh = 0)
```
...
`mom_hs=0: kid_score = -11 + 51*0 + 1.1* mom_iq + 0.5*0*mom_iq`

`                    = -11 + 1.1*mom_iq`

</br>

`mom_hs=1: kid_score = -11 + 51*1 + 1.1* mom_iq + 0.5*1*mom_iq`
...
- Der *Achsenabschnitt* (`Intercep`) gibt den geschätzten IQ des Kindes an, wenn man eine Mutter mittlerer Intelligenz und ohne Schulabschluss betrachtet.
...
## Perzentilintervalle aus der Posterori-Verteilung, de luxe

Mit dem Paket `bayestestR` kann man sich komfortabel die 95%-Perzentilintervalle ausgeben lassen (hier als *Equal Tail Intervals* ETI, bezeichnet):
```{r echo = TRUE}
library(bayestestR)
ci(m10.5)
```

Alternativ kann man mit `quantile()` das Gleiche erreichen.

Highest Density (Posterior) Intervalle (HDI oder HDPI) kann man sich ebenso komfortabel ausgeben lassen mit `hdi(m10.5)`. 
