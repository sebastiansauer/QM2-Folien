...
plot_post_41 <- 
  ...
  labs(x = expression(mu),
       title = TeX("Posteriori-Verteilung für $\\mu$, m41")) +
...
- Der Mittelwert $\mu$ für jede Post-Verteilung ergibt sich aus dem .green[linearen Modell (der Regression)].
...
- Außerdem brauchen wir einen .blue[Priori-Wert], der die Streuung $\sigma$ der Größe (`height`) angibt.
...
```{r fig.asp = 1, fig.width=2, echo = FALSE}
d <-
  tibble(
    x = seq(0, 5,.1),
    y = dexp(x, rate = 1)
...
```{r ref.label = "dexp-plot", echo = FALSE, eval = TRUE}
...
text_df <-
  tibble(weight = c(34, 43),
         ...
         label  = c("Embryo", "Größter Mensch (272 cm)"))
