...
d %>% 
  ...
  geom_segment(data = . %>% 
               ...
               aes(xend = p_grid, yend = 0, size = post),
  ...
  scale_x_continuous("Wahrscheinlichkeit Wasser (pi)", breaks = c(0:10) / 10) +
...
samples3 <-
  d %>% 
  slice_sample(n = n_samples, weight_by = post, replace = T) %>% 
