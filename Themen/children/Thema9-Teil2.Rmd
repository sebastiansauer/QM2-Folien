...
\mathcal{log}(p_i) &= \alpha + \beta \cdot \text{height}_i \\
...
Eigentlich hatten wir das schon hier gesehen:

```{r}
posterior_interval(m92, prob = .95,pars = "height_c")
```
...
`posterior_predict()` zeigt uns die Vorhersagen aus der PPV für $\alpha + \beta \cdot 10$.
...
\text{log}(p_i) &= \alpha + \beta \cdot r_i \\
...
```{r ref.label = "hallodri-plot", fig.asp = .618} 
