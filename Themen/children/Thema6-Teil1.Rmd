...
*Forschungsfrage:* Unterscheidet sich der mittlere IQ-Wert (`kid_score`) von Kindern in Abhängigkeit davon, ob ihre jeweilige Mutter über einen Schlusabschluss (`mom_hs`) verfügt?

.pull-left[
```{r echo = TRUE, results = "hide"}
library(rstanarm)
data("kidiq")
m10.1 <- stan_glm(kid_score ~ mom_hs, data = kidiq)
```
...
- In der Bayes-Statistik betrachtet man dazu meist ein PI (z.B. mit 95%) der Posteriori-Verteilung.
]
...
plot(m10.1, 
     ...
     plotfun = "mcmc_hist") +
  geom_vline(
    xintercept = pi_mom_hs$pi_95) +
  labs(
    title = "95%-PI zum Mittelwertsunterschied"
  )
```
