...
Datensatz `samples`, $n=10^4$, $k=100$ Gitterwerte, basierend auf dem [Modell oben](#posttab).
...
samples_zwielicht <- 
  tibble(
    gewinnchance_muenze = sample(
      d_zwielicht$p_grid , 
      prob=d_zwielicht$posterior, size=1e4, replace=TRUE)) %>% 
  mutate(
    id = row_number())
...
Die *Posteriori-Verteilung* gibt die Wahrscheinlichkeit jedes Parameterwerts $p$ wider, gegeben der empirischen Daten $D$: $Pr(p|D)$. M
