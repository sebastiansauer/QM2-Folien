...
```{r Kung-11}
knitr::include_graphics("https://github.com/sebastiansauer/QM2-Folien/raw/main/img/Lik-Prior.png")
```
...
$\sigma \sim \mathcal{E}(\lambda=0.1)$
...
d_grid <-
  ...
  crossing(mu    = seq(from = 140, to = 160,
                       length.out = n),
           sigma = seq(from = 4, to = 9, 
                       length.out = n))

grid_function <- function(mu, sigma) {
  
  dnorm(d2$height, mean = mu, 
        sd = sigma, log = T) %>% 
    sum()
...
d_grid <-
  ...
  mutate(prior_mu    = dnorm(mu, mean = 178, 
                             sd = 20, log = T),
         prior_sigma = dunif(sigma, min = 0, 
                             max = 50, log = T)) %>% 
  mutate(product = 
           log_likelihood + prior_mu + prior_sigma) %>% 
  mutate(probability = 
           exp(product - max(product)))
```



```{r Kung-19, eval = FALSE}
d_grid %>% 
