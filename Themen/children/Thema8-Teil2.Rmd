...
```{r hauspreis-stratifizieren, child="Hauspreis-stratifizieren.Rmd"}
...
💡 Durch das Aufnehmen von Prädiktoren in die multiple Regression werden die Prädiktoren *kontrolliert*:
...
```{r konfundierer-kontrollieren, child="Konfundierer-kontrollieren.Rmd"}
...
```{r fig.asp = .5, fig.width=7}
km2 <- mediation_triangle(x = "livingArea",
                          y = "price",
                          m = "bedrooms") %>% 
  ggdag_dconnected(text = FALSE, use_labels = "label") +
  theme_dag()
...
## Jedes Kausalmodell impliziert Unabhängigkeiten
