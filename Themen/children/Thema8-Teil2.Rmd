...
### Mit 4 statt 2 Schlafzimmer steigt der Wert auf 250k
...
💡 Durch das Aufnehmen von Prädiktoren in die multiple Regression werden die Prädiktoren *kontrolliert* ("adjustiert"):
...
`d_connected` heißt, dass die betreffenden Variablen "verbunden" sind durch einen Pfad, durch den die Assoziation (Korrelation) wie durch einen Fluss fließt. `d_separated` heißt, dass sie nicht `d_connected` sind.
...
.pull-left[
- DAGs sind eine bestimmte Art von Kausalgraphen.

- Ein *Graph* besteht aus Knoten (Variablen) und Kanten (Linien), die die Knoten verbinden.

- DAGs sind eine Methode, um Kausalstrukturen graphisch zu analyiseren.
