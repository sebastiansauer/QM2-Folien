...
Nein! Statistiken in Beobachtungsstudien dürfen nicht direkt kausal interpretiert werden.
...
## Münzen werfen als Baum: nach 2 Würfen


```{r muenze1-mermaid, eval = FALSE}
m1 <-
mermaid("
        graph LR
        A(0)-->|1/2| B(K)
...
plotly::export(m1, file = "img/muenz1.pdf")
```
Wir zählen, wie oft Kopf (K) geworfen wird:
...
## Münzen werfen als Baum: nach 3 Würfen

Wir zählen, wie oft Kopf (K) geworfen wird:

```{r, out.width="90%", fig.asp = 2, echo = FALSE}
nomnoml("
#direction: down
[<start> start] -> 1/2[K]
[<start> start] -> 1/2[Z]
